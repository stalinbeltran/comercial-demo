"""
reportes/ventas_por_categoria/fill_resumen_ventas.py
...
Uso:
    ...
    python reportes/ventas_por_categoria/fill_resumen_ventas.py --desde 2025-01-01 --hasta 2025-12-31
    python reportes/ventas_por_categoria/fill_resumen_ventas.py --sucursal 3
    python reportes/ventas_por_categoria/fill_resumen_ventas.py --todos
"""
...
GROUP BY categoria, subcategoria, producto_id, sku, producto
"""
...
WHERE estado_pedido NOT IN ('cancelado', 'anulado', 'borrador')
  ...
  AND fecha_pedido <  DATE_ADD(%s, INTERVAL 1 DAY)
  AND sucursal_id = %s
GROUP BY categoria, subcategoria, producto_id, sku, producto, sucursal_id, sucursal_nombre
"""

_SQL_SUCURSALES = """
SELECT DISTINCT sucursal_id AS id, sucursal_nombre AS nombre
FROM linea_venta WHERE sucursal_id IS NOT NULL ORDER BY sucursal_id
"""
...
ON DUPLICATE KEY UPDATE
    sucursal_nombre=VALUES(sucursal_nombre), categoria=VALUES(categoria),
    subcategoria=VALUES(subcategoria), sku=VALUES(sku), producto=VALUES(producto),
    unidades_vendidas=VALUES(unidades_vendidas), precio_promedio=VALUES(precio_promedio),
    total_descuentos=VALUES(total_descuentos), ingresos_netos=VALUES(ingresos_netos),
    num_pedidos=VALUES(num_pedidos), fecha_actualizacion=NOW()
"""
...
def fill_total(src, tgt, desde, hasta):
    ...
    rows = [{**r, "periodo_inicio": desde, "periodo_fin": hasta,
             "sucursal_id": 0, "sucursal_nombre": None} for r in cur.fetchall()]
    cur.close()
...
def main():
    hoy = date.today()
    p = argparse.ArgumentParser(description="Fill resumen_ventas")
    ...
    if args.todos:
        ...
        for s in sucursales:
            ...
            print(f"  sucursal {s['id']:>3} — {s['nombre']:<30} {n:>4} filas")
            total += n
        print(f"\n  total: {total} filas")
